...
```
--切片
切片和内存操作息息相关，其中隐藏了很多细节，需要注意
1.函数传递切片，是传递地址（可以理解为指针），所以在函数中改变其值或者其指向的值，都会影响函数外的值
2.切片操作(形如a:=b[1:2])，是传递地址，并且这两个变量共享一块地址，操作任何一个变量，都会影响另一个变量。
虽然指向同一块，但是a!=b,b的cap值比a的cap值少1，且a[0]的值不会被b影响
3.append操作，其返回值也是个切片，与其参数(append函数的第一个参数，也就是切片类型)共享同一块地址。
a=append(b, 1),此时a!=b，且len(b)+1==len(a),若len(b)+1<=cap(b),此时a,b互相影响，否则互不相干（第4条)
4.当append操作会超出切片(append的第一个参数)的容量(cap)时，其返回值是切片，但是是一块新的内存地址
5.copy操作，其两个参数因为需要显式分配内存，所以没有隐式的内存操作
